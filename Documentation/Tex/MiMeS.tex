...
%
\begin{itemize}
	\item {\tt LD heff(LD T)}: $\heff$ as a function of $T$.
	\item {\tt LD geff(LD T)}: $\geff$ as a function of $T$.
	% 
	\item {\tt LD dheffdT(LD T)}: $d\heff/dT$ as a function of $T$.
	\item {\tt LD dgeffdT(LD T)}: $d\geff/dT$ as a function of $T$.
	%
	\item {\tt LD dh(LD T)}: $\delta_h = 1 + \frac{1}{3} \frac{d\log \heff}{d\log T}$ as a function of $T$.
	%
	\item {\tt LD s(LD T)}: The entropy density of the plasma as a function of $T$.
	\item {\tt LD rhoR(LD T)}: The energy density of the plasma as a function of $T$.
	\item {\tt LD Hubble(LD T)}: The Hubble parameter assuming radiation dominated expansion as a function of $T$.
\end{itemize}

Moreover, there are several cosmological quantities are given as ({\tt constexpr}) static members:
%
\begin{itemize}
	\item {\tt LD T0}: CMB temperature today~\cite{Zyla:2020zbs} in $\GeV$.
	\item {\tt LD h\_hub}: Dimensionless Hubble constant~\cite{Zyla:2020zbs}.
	\item {\tt LD rho\_crit}: Critical density~\cite{Zyla:2020zbs}.
	\item {\tt LD relicDM\_obs}: Central value of the measured DM relic abundance~\cite{Planck:2018vyg}.
	\item {\tt LD mP}: Planck mass~\cite{Zyla:2020zbs} in $\GeV$.
\end{itemize}

\subsection{{\tt AnharmonicFactor} class}
%
The class {\tt mimes::AnharmonicFactor<LD>} is responsible interpolating the anharmonic factor as defined in \eqs{eq:anharmonic_f}. The corresponding header file is {\tt \mimes/src/AnharmonicFactor/AnharmonicFactor.hpp}. 
...
%
The {\tt mimes::AxionMass<LD>} class is responsible for the definition of the axion mass. The header file of this class is {\tt \mimes/src/AxionMass/AxionMass.hpp}. Its usage and member functions are described in the examples given in sections~\refs{sec:First_examples,sec:complete_examples}. However, it would be helpful to outline them here.
...
\end{cpp}
%
Here {\tt chi\_PATH} is the path to a data file that contains two columns; $T$ (in $\GeV$) and $chi$ (in $\GeV^4$), with increasing $T$. The arguments {\tt minT} and {\tt maxT} are the interpolation limits. These limits are used in order to stop the interpolation in the closest temperatures that exist in the data file. That is the actual interpolation limits are $T_{\min}\geq${\tt minT} and $T_{\max}\leq${\tt maxT}. Beyond these limits, by default, the axion mass is assumed to be constant. However, this can be changed by using the member functions
%
\begin{cpp}
...
%
\begin{cpp}
	void set_ma2(std::function<LD(LD,LD)> ma2)
\end{cpp}

\subsection{{\tt AxionEOM} class}
%
The {\tt mimes::AxionEOM<LD>} class is not useful for the user. However, it is responsible for the interpolation of the underlying cosmology, and the definition of the axion EOM~\ref{eq:eom_sys}, which is passed to the ODE solver of {\tt NaBBODES}. 
...
%
\begin{cpp}
	void operator()(std::array<LD,2> &lhs, std::array<LD,2> &y, LD u)
\end{cpp}
%
...
%
The {\tt mimes::Axion<LD,Solver,Method>} class is the class that combines all the others, and actually solves the axion EOM~\ref{eq:eom_sys}. Its header file is {\tt \mimes/src/Axion/AxionSolve.hpp}, and its constructor is
%
\begin{cpp}
	mimes::Axion<LD,SOLVER,METHOD<LD>> Ax(theta_i, fa, umax, TSTOP, ratio_ini, 
...
Once this function finishes, we have access to all the relevant results in the member variables
%
\begin{itemize}
 	\item {\tt std::vector<std::vector<LD>> points}: The quantities $a/\ai, \ T, \ \theta, \ \zeta, \rho_a$, at the integration  steps.
	\item {\tt std::vector<std::vector<LD>> peaks}: The quantities $a/\ai, \ T, \ \theta, \ \zeta, \rho_a, \ J$, at the peaks of the oscillation. Note taht these points are computed using linear interpolation between two integration points with a change in teh sign of $\zeta$.
	\item {\tt std::vector<LD> dtheta}: Local integration error for $\theta$.
	\item {\tt std::vector<LD> dzeta}: Local integration error for $\zeta$.
	\item {\tt LD T\_osc}: The oscillation temperature. 
	\item {\tt LD theta\_osc}: The angle at $T=\Tosc$.
	\item {\tt LD a\_osc}: $a/\ai$ at $T=\Tosc$.
	\item {\tt LD gamma}: The amount of entropy injection between the last peak ($T=T_{\rm peak}$) and today ($T=T_0$).
\end{itemize}
...
In case the mass of the axion is changed, we also need to remake  the interpolation (\ie run mimes::AxionEOM::makeInt()). This is done using
%
\begin{cpp}
	void restart()
\end{cpp}
...
Finally, there is static {\tt mimes::Cosmo<LD>} member variable
%
\begin{cpp}
	 static {\tt mimes::Cosmo<LD>} plasma
\end{cpp}
%
This variable can be used without an instance of the {\tt mimes::Axion<LD,Solver,Method>} class.
...
\begin{table}[h!]
	...
	\begin{tabular}{l l}
		...
		{\tt axionMass} & \multirow{1}{12cm}{ Instance of {\tt mimes::AxionMass<LD>} class. In \CPP this instance is passed as a pointer to the constructor
		of the {\tt mimes::Axion<LD,Solver,Method>} class, while in \PY it is simply passed as a variable.}\\ \\  \\ \\
		\hline\\[-0.4cm]
