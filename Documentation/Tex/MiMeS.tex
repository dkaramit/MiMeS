...
\subsection{{\tt Cosmo} module}
%
Inside The {\tt Cosmo} module defines the \pyin{Cosmo} class, which contains information about the plasma. The relevant shared library ({\tt lib/libCosmo.so}) is obtained by by running {\tt make lib/libCosmo.so}. It is important to note that, the interpolation of the various functions are created once the module is imported, and not for each instance. That is, the input file and interpolation range can be changed by changing the first, second and third arguments of the constructor, respectively, in {\tt src/Cosmo/Cosmo.cpp}. If these values change, then the library must be compiled.
%
