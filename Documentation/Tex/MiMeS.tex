...
\end{equation}
%
which is solved by $\theta = \thetai + C \dint_{0}^t d t \ \lrb{ \dfrac{a_{0}}{a(t)} }^3$, with $\thetai$ some constant. That is, as the Universe expands, $\theta \approx \thetai$. Since we would like to calculate the relic abundance of axions, we can integrate \eqs{eq:eom} from a time after inflation (call it $t = \ti$) such that $ \dot \theta |_{t=\ti} = 0$ and  $\theta|_{t=\ti} \approx \thetai$.   
...
\setcounter{equation}{0}
%
\mimes makes only a few, fairly general, assumptions. First of all, it is assumed that the axion energy density is always subdominant compared to radiation or any other component of the Universe, and that decays and annihilations of particles have a negligible effect on the axion energy density. Moreover, the initial condition is always assumed to be $\thetai = const$ (\ie $\dot \theta|_{t=\ti}=0$). Moreover, it is also assumed that the axion mass deceases monotonically at high temperatures. 


\paragraph{User input at compile-time} \DK{Describe the files for the plasma related quantities, the mass of the axion, and the anharmonic factor.
Mention optimization options, and {\tt g++} and {\tt clang} seem to work. Describe the available solvers (appendix for more details).}.


\paragraph{User input at run-time} \DK{Since you have described the classes, you can now talk about {\tt ratio\_ini} and the others! Mention the solver parameters and redirect to the appendix for more info.}.
