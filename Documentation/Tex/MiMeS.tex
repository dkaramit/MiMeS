...
%
There are several examples in \CPP ({\tt MiMeS/UserSpace//Cpp}) and \PY ({\tt MiMeS/UserSpace//Python}), as well as \JUPY  notebooks ({MiMeS/UserSpace//JupyterNotebooks}), that show in detail how \mimes can be used. 

\subsubsection{Using \mimes in \CPP} 
%
In order to use the {\tt mimes::Axion} class in a \CPP program, we need ti include the header file {\tt AxionSolve.hpp} located in {\tt MiMeS/src/Axion}. That is, on top of the {\tt .cpp}, we need to write 
%
\begin{cpp}
...
\end{py}
%
It is important that \mintinline{python}{'path_to_src'} provides the relative path to the {\tt src} directory. For example, if the script is located in {\tt MiMeS/UserSpace//Python}, \mintinline{python}{'path_to_src'} should be \mintinline{python}{'../../src'}.
...
%
Also, it is assumed that the entropy is resumes its conserved status at a temperature higher than the minimum temperature of  {\tt inputFile} (which is required by the constructor of the {\tt mimes::Axion} class).  
%
...
\subsection{Options at Compile-time}\DK{Talk about the options, {\tt LONG} and its relation to LD, {\tt solver}, {\tt METHOD} ...}

\subsection{User input}\label{sec:input}
%
\subsubsection{Compile-time input} 
%
...
\begin{itemize}
	...
	If the user needs the axion mass to be constant, this file should contain only one line that contains a temperature -- its importance will become clear in the next paragraph -- and a value of $\chi$. 
\end{itemize}
%
The paths to these files should be given at compile time. That is, once {\tt Definitions.mk} changes, \run{bash configure.sh} and then \run{\tt make} must be ran. The user can change the content of the data files (without changing their paths), in order to use them without compiling \mimes again. However, the user has to make sure that all the files are sorted so that the values of first column increase (with no duplicates or empty lines). In order to ensure this, it is advised to run \run{bash FormatFile.sh path-to-file} (in Appendix~\ref{app:util} there are some details on {\tt MiMeS/src/FormatFile.sh}), in order to format the file (that should exist in \run{path-to-file}) complies with the requirements of \mimes.

\paragraph{Static variables} The header file {\tt MiMeS/src/static.hpp} contains definitions of various static variables that are used in the \PY modules and the {\tt mimes::Axion} class. These variables are instances of the classes {\tt mimes::Cosmo}, {\tt mimes::AxionMass}, and {\tt mimes::AnharmonicFactor}, and they are responsible for interpolations of the various plasma parameters (\ie $\heff(T)$, $\geff(T)$, $s(T)$, etc.), the axion mass, and the anharmonic factor, respectively.

The constructors of all these classes need the data files whose paths are provided by the user in {\tt Definitions.mk} (in the root directory of \mimes). Moreover, the user is allowed to choose the minimum and maximum interpolation temperature for the classes {\tt mimes::Cosmo}, {\tt mimes::AxionMass}. \mimes uses, by default, the temperature range $T \in [0,m_P]$,~\footnote{Here $m_P = 1.22 \times 10^{19}~\GeV$, is the Planck mass.} however, this can easily be changed.
%
This can be done by changing the second and third argument of these two classes, in order to change the minimum and maximum temperature, respectively. For example, in order to interpolate the plasma parameters between $T_{\rm min} = 1~\GeV$ and $T_{\rm max} = 100~\GeV$, we just change the declaration of the {\tt cosmo} variable form
%
\begin{cpp}
...
to 
\begin{cpp}
	static mimes::Cosmo<LD> cosmo(cosmo_PATH,1,100);
\end{cpp}
% 
Since beyond these limits, $\heff$ and $\geff$ are assumed to be constant, one has to be careful with the choice of the limits as they might severely affect the accuracy of the results. 

Similarly, in order to change the interpolation limits of the {\tt axionMass} variable (instance of {\tt mimes::AxionMass}) to $T_{\rm min} = 1~\GeV$ and $T_{\rm max} = 100~\GeV$, we need to change (notice that the second template argument is set to {\tt true}, which indicates that a data file is expected)
%
\begin{cpp}
	static mimes::AxionMass<LD,true> axionMass(chi_PATH,0,mimes::Cosmo<LD>::mP);
\end{cpp} 
%
to 
\begin{cpp}
	static mimes::AxionMass<LD,true> axionMass(chi_PATH,1,100);
\end{cpp}
%
Beyond these limits, the axion mass is assumed to be
%
\begin{equation}
	\maT = \Bigg\{
	\begin{matrix}
...
\end{equation}
%
Therefore, changing the limits can cause inaccurate results. The user can change this definition, by calling 
%
\begin{cpp}
	axionMass.set_ma2_approx(ma2_MIN, ma2_MAX);
\end{cpp}
%
where \run{ma2\_MIN} and \run{ma2\_MAX} are function objects~\footnote{Anything that can be {\tt std::function<LD(LD,LD)>}.} -- with signatures {\tt LD ma2\_MIN (LD T, LD fa)} and {\tt LD ma2\_MAX (LD T, LD fa)}, with {\tt LD} being {\tt double} or {\tt long double} -- that correspond to the limits $T<T_{\rm min}$ and $T>T_{\rm max}$, respectively. Alternatively, since \run{ma2\_MIN} and \run{ma2\_MAX} are  {\tt public} lambda functions of the {\tt mimes::AxionMass} class, one can assign other lambda functions to them. This, one has access to the corresponding instance, and its variables (in Appendix~\ref{app:classes} in order to we list all the variables of this class (\DK{write about \run{this->chiMAX} etc in the Appendix}).   


\paragraph{Important: User defined function of axion mass} The user can define their own axion mass function, without interpolating data form a file, by changing the  \run{axionMass} instance (found in {\tt MiMeS/src/static.hpp}) to 
%
\begin{cpp}
	static mimes::AxionMass<LD,false> axionMass(ma2);
\end{cpp}
%
where {\tt ma2} -- the axion mass squared in $\GeV^2$ -- is function object with signature {\tt LD ma2 (LD T, LD fa)}. Notice also that the second template argument in this case is \run{false}, which indicates that a data file will not be used.

\paragraph{Important: The limits are suggestions} If the chosen interpolation limits exceed the minimum and maximum temperatures in the data files, the latter will be used as $T_{\rm min}$ and $T_{\rm max}$.
...
\paragraph{A complete \CPP example}\DK{Give the code with explanation of {\tt MiMeS/UserSpace//CPP/Axion}, along with the compilation options.}
