...
The underlying cosmological scenario, in terms of the parametrisation of ref.~\cite{Arias:2020qty}, is defined through the parameters
%
\begin{eqnarray}
...
\subsection{Results}
%
In both cases one should get the same results (if the same numeric type, and RK parameters and method are used), which should be similar to \Figs{fig:results}. It should be noted that this particular example is chosen such that the axion relic abundance is close to its observed value~\cite{Planck:2018vyg} ($\Omega h^2 \approx 0.12$), and is indicative of the features one can expect if entropy is injected to the plasma fot $T \ll \Tosc$.
%
\begin{figure}[h]
	\begin{subfigure}[]{0.5\textwidth}
		\includegraphics[width=1\textwidth]{figs/theta_evolution.pdf}
		\caption{}
		\label{fig:theta_evolution}
	\end{subfigure}
	\begin{subfigure}[]{0.5\textwidth}
		\includegraphics[width=1\textwidth]{figs/zeta_evolution.pdf}
		\caption{}
		\label{fig:zeta_evolution}
	\end{subfigure}
	\begin{center}
		\begin{subfigure}[]{0.5\textwidth}
			\includegraphics[width=1\textwidth]{figs/axion_energy_density.pdf}
			\caption{}
			\label{fig:axion_energy_density}
		\end{subfigure}
	\end{center}
	\caption{...}
	\label{fig:results}
\end{figure}
%
Moreover, the relative (local) error of integration is shown in \Figs{fig:local_errors}


%
\begin{figure}[h]
	\includegraphics[width=1\textwidth]{figs/local_errors.pdf}
	\caption{}
	\label{fig:local_errors}
\end{figure}
