...
\begin{enumerate}
	...
	\end{itemize}
	This variable defines a macro (with the same name) that is passed as the third template parameter of \cppin{mimes::Axion<LD,Solver,Method>} (\ie {\tt METHOD<LD>} in the place of {\tt Method}). 
	%
...
\setcounter{equation}{0}
We present tables~\refs{tab:run_time-input,tab:input,tab:template-arguments}, with the various available run-time inputs, required files, and template arguments. In table~\ref{tab:compile_time-input} we show the available compile-time options, that can be used when compiling using the various {\tt makefile} files. 
%
\begin{table}[h!]
	\centering
	\begin{tabular}{l l}
