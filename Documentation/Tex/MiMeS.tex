...
\subsection{Options at Compile-time}\DK{Talk about the options, {\tt LONG}, {\tt solver}, {\tt METHOD} ...}
...
Similarly, in order to change the interpolation limits of the {\tt axionMass} variable (instance of {\tt mimes::AxionMass}) to $T_{\rm min} = 1~\GeV$ and $T_{\rm max} = 100~\GeV$, we need to change
%
\begin{cpp}
...
to 
\begin{cpp}
	static mimes::AxionMass<LD> axionMass(chi_PATH,1,100);
\end{cpp}
%
...
%
\begin{cpp}
	static mimes::AxionMass<LD> axionMass(ma2);
\end{cpp}
%
where {\tt ma2} -- the axion mass squared in $\GeV^2$ -- is function object with signature {\tt LD ma2 (LD T, LD fa)}.
