...
\begin{frame}{\insertsubsectionhead}
		...
		\end{eqnarray*}\\[-0.2cm]
		%
		The parameter $u$ is ``time''~\footnote{\fontF With $\ai$ the initial value of the scale factor. It corresponds to $\Ti$.} $u \equiv \log \lrb{a/\ai}$. \pause\\[0.5cm]

		\begin{itemize}
			\item Integration starts at $\Ti$ such that $3H(\Ti)/\ma(\Ti) = x \gg 1$, with a u ser defined $x$ ($\Ti$ is automatically determined).
