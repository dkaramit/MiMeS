...
### The ```AxionMass``` class
The ```AxionMass``` class is defined in the module with the same name that can be found in the directory ```MiMeS/src/interfacePy/AxionMass```. This class is responsible for the definition of the axion mass. This module loads the corresponding shared library from ```MiMeS/lib/libma.so```, which is created by compiling ``` MiMeS/src/AxionMass/AxionMass.cpp``` using ```make lib/libma.so```. Its usage is described in the <a href="Examples.html">Examples</a>. Moreover, this class is used in the same way as ```mimes::AxionMass< LD >```. However, we should append in this section the definition of its member functions in ```python``` .
